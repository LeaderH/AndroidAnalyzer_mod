    ...
    def load_to_output_list(self, args) :
        ...
        if stopwatch_total_elapsed_time and stopwatch_analyze_time :
            ...
            if (REPORT_OUTPUT == TYPE_REPORT_OUTPUT_ONLY_FILE) :
                self.output_and_force_print_console("Analyzing time: " + str(stopwatch_analyze_time) + " secs")
                self.output_and_force_print_console("Total elapsed time: " + str(stopwatch_total_elapsed_time) + " secs")
            else:
                self.output("Analyzing time: " + str(stopwatch_analyze_time) + " secs")
...
class EfficientStringSearchEngine :
    ...
    def search(self, vm, allstrings_list) :
        ...
        for stringData in vm.get_string_data_item():
            dict_string_value_to_idx_from_file_mapping[stringData.get_off()]=stringData.get_data()
...
def get_hash_scanning(writer) :
    ...
    tmp_hash = hashlib.sha512(tmp_original.encode("utf-8")).hexdigest()
    return tmp_hash
